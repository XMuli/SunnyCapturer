...
**亮点**
  ...
  v5.3.0 重点完善了多语言体验、语言切换能力，以及设置与更新相关的稳定性
  问题。这个版本进一步提升了国际化覆盖范围，让更多界面文案能够正确跟随语
  言切换实时更新。

**新增与优化**

  - 支持更完善的多语言适配与翻译更新。
  - 改进语言切换体验，更多界面文案现在可以在切换语言后正确刷新。
  - 补齐了更多窗口标题、托盘菜单、文件对话框筛选器等界面文本的国际化支
    持。
  - 持续更新并优化多语言翻译内容，提升整体一致性。
  - 清理了一部分历史遗留的未完成翻译项。
...
**修复**
  ...
  - 修复设置窗口在多屏环境下的几何位置恢复问题。
  - 修复更新检查逻辑中的多个问题，并优化默认配置行为。
  - 修复部分多语言文案未正确接入翻译系统的问题。
  - 修复部分界面在语言切换后文案不同步的问题。

**其他改进**

  - 更新了部分构建与打包脚本，便于 `v5.3.0` 版本发布与分发。
  - 更新 QHotkey 子模块到较新的提交版本。
  - 进行了一些内部整理与维护性优化。

**说明**

  - 如果你发现某些文案未翻译、翻译不自然或切换语言后未即时刷新，欢迎继续
      反馈。
...
**优化改进**
  ...
  - **本地化** — 更新了日语（ja_JP）和俄语（ru_RU）翻译；
    移除标记为 `[type="unfinished"]` 的未完成条目，提升翻译质量。
...
感谢大家的一直的热情支持和期待，今天 v5.0.0 版本正式发布🎉🎉🎉
...
- 修复少部分无法开启自启 #83
- 修复一些发现的小缺陷等
- 更新葡萄牙-巴西语言翻译，感谢 @thiagodalsoto

  

Note: 录制和 GIF 功能，选中区域后，手动按下 `Enter` 按键可看到控制面板以及计时器

📢PS: GPU  仅支持 NVIDIA 显卡，必须 CUDA >= 12.0.0（低版本未测试），推荐最新版，官网下载地址 [cuda-toolkit-archive](https://developer.nvidia.com/cuda-toolkit-archive)；Linux 版本的安装包，会在稳定后将会一起发布。
