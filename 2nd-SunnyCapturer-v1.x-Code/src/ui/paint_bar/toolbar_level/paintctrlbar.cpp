...
void PaintCtrlBar::onCbbFromCurrentTextChanged(const QString &text)
{
    ...
    QString from;
    if (iamge_translate_channel == 0) {
        from = QString::fromStdString(languageMapCodeBaiDu().at(text).toStdString());
        CJ_SET("tokens.iamge_translate.baidu.from", from.toStdString());
    } else if (iamge_translate_channel == 1) {
        from = QString::fromStdString(languageMapCodeYouDao().at(text).toStdString());
        CJ_SET("tokens.iamge_translate.youdao.from", from.toStdString());
    }

    m_imgTransData.from_baidu = from;
    emit sigImgTranslate(m_imgTransData);
}
...
void PaintCtrlBar::onCbbToCurrentTextChanged(const QString &text)
{
    ...
    QString to;
    if (iamge_translate_channel == 0) {
        to = QString::fromStdString(languageMapCodeBaiDu().at(text).toStdString());
        CJ_SET("tokens.iamge_translate.baidu.to", to.toStdString());
         m_imgTransData.to_baidu = to;
    } else if (iamge_translate_channel == 1) {
        to = QString::fromStdString(languageMapCodeYouDao().at(text).toStdString());
        CJ_SET("tokens.iamge_translate.youdao.to", to.toStdString());
         m_imgTransData.to_youdao = to;
    }
